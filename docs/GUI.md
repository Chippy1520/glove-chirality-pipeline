...
- **Scan Cameras** uses the existing Windows backend fallback, reports the default mode, and probes whether 1920x1080 MJPG is available. It releases each test capture immediately. The GRIP 1080p preset requests DirectShow, MJPG, and 1920x1080 before the first frame, and live capture asks for a one-frame driver buffer. Camera FPS can be Auto, 15, 20, 25, 30, or 60; the choice is requested before the first frame. A box is not a new glove until a second observation confirms it. Matching uses buffered overlap, the belt direction, and the lane. A box already past the trigger line does not start a new track. A crop must be clear of the frame border and of another glove. Detection runs on its own stage while the previous glove is still in Layer 2. Identity is tracked across the ROI, not only inside the trigger box. A weak detection can keep a passage alive but cannot create one. If two gloves were already separate and their masks then merge, `merge_recovery` keeps both tracks and still sends one bbox crop each. A wrinkled glove that was never that far apart stays one crop. `merge_recovery: false` restores the single-blob skip. `tracker_mode: legacy` restores the previous extractor. YOLO `imgsz` stays 640. The operator decision card updates from a small endpoint about 10 times a second. Diagnostics and passage history stay on the slower refresh. The browser preview is a cached display JPEG; it does not draw or encode on the detector thread. If the camera negotiates a different size, Factory Live shows a geometry mismatch and does not upscale the frame.
