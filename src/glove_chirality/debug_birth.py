...
def debug_video(
    ...
    *,
    ...
    try:
        ...
        processor.close(frame_index / fps if frame_index >= 0 else 0.0)
    finally:
        capture.release()
    tracker = processor._byte_tracker
    events = [] if tracker is None else list(tracker.debug_events)
    return _report(notes, events, config)
...
def _report(notes: list[_BoxNote], events: list[dict[str, object]], config: ExtractionConfig) -> str:
