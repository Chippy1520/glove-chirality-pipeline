...
def save_calibration_preview(video: str | Path, output: str | Path, config: ExtractionConfig, seconds: float = 0.0):
    ...
    for detection in build_detector(config.detector).detect(frame):
        cv2.rectangle(frame, (detection.x1, detection.y1), (detection.x2, detection.y2), (0, 0, 255), 3)
        cv2.putText(frame, f"{detection.confidence:.2f}", (detection.x1, detection.y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    output = Path(output)
