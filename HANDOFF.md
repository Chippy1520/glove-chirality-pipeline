...
- Responsive Flask/Waitress browser workstation for Layer 1 configuration, extraction, Layer 2 training/inference, concurrent local TensorBoard, explanations, historical split-aware comparison, and authenticated read-only LAN monitoring. The legacy Tkinter app remains under `glove-pipeline-tk` during migration.
