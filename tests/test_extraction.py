...
def _synthetic_video(path: Path):
    ...
    for index in range(100):
        ...
        if 10 <= index < 75:
            y = -50 + (index - 10) * 4
            cv2.rectangle(frame, (120, y), (200, y + 85), (35, 35, 35), -1)
            for finger in range(4):
                x = 121 + finger * 20
                cv2.rectangle(frame, (x, y - 22), (x + 14, y + 8), (35, 35, 35), -1)
        writer.write(frame)
    writer.release()


def test_extracts_exactly_one_event_and_manifest(tmp_path):
    video = tmp_path / "left_sample.avi"
    _synthetic_video(video)
    config = ExtractionConfig(
        detector=DetectorConfig(roi=(0, 0, 1, 1), trigger_zone=(0.15, 0.15, 0.85, 0.85), dark_threshold=90, min_area_ratio=0.008, max_area_ratio=0.5, morph_kernel=5),
