...
- **Scan Cameras** uses the existing Windows backend fallback, reports the default mode, and probes whether 1920x1080 MJPG is available. It releases each test capture immediately. The GRIP 1080p preset requests DirectShow, MJPG, and 1920x1080 before the first frame, and live capture asks for a one-frame driver buffer. YOLO `imgsz` stays 640. The browser preview is a cached display JPEG; it does not draw or encode on the detector thread. If the camera negotiates a different size, Factory Live shows a geometry mismatch and does not upscale the frame.
